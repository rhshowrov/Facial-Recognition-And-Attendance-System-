...
def tkAtt(request,course_name, section_number):
    ...
    while True:
        ...
        for i, face in enumerate(faces):
            ...
            cv2.imwrite(face_image_path, cv2.cvtColor(face_img, cv2.COLOR_RGB2BGR))  # Save face image
...
def check(student_name, course_name, section_number):
    ...
    try:
        ...
        if is_assigned:
            ...
            Attendance.objects.create(student=Student.objects.get(name=student_name), course=course)
            attendance_status = "Recorded"
            
            # Append assignment status to assignments list
            assignments.append({'student_name': student_name, 'is_assigned': is_assigned, 'attendance': attendance_status})
            
            # Print assignment status
            print(f"{student_name} is assigned to {course_name} - Section {section_number}. Attendance: {attendance_status}")
        else:
            assignments.append({'student_name': student_name, 'is_assigned': is_assigned, 'attendance': "Not Recorded"})
            print(f"{student_name} is not assigned to {course_name} - Section {section_number}")
